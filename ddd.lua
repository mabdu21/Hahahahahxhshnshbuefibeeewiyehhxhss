...
local AutoBlock = {
    ...
    -- ตั้งค่าหลัก
    MinDistance = 20,      -- ระยะสำหรับการโจมตีปกติ (Attack Sounds + Charge Anim)
    ThrowDistance = 80,    -- ระยะสำหรับ Throw Attack
    
    Cooldown = 0.25,       -- Cooldown หลัก (ลดลงเล็กน้อยเพื่อความไว)
    
    CurrentKey = Enum.KeyCode.X,  -- Key สำหรับ Block (สามารถเปลี่ยนได้)
...
function AutoBlock.IsKillerLookingAtMe(killerHRP)
    ...
    return angle < math.rad(50)  -- เพิ่มเป็น 50 องศา เพื่อความยืดหยุ่น
end
...
function AutoBlock.ExecuteBlock()
    pcall(function()
        ...
        local Warp = Modules:FindFirstChild("Warp")
        if not Warp then return end
        local Index = Warp:FindFirstChild("Index")
        if not Index then return end
        local Event = Index:FindFirstChild("Event")
...
function AutoBlock.ExtractNumericSoundId(sound)
    ...
    return string.match(sid, "rbxassetid://(%d+)") or string.match(sid, "://(%d+)") or string.match(sid, "^(%d+)$")
end
...
function AutoBlock.IsKiller(character)
    ...
    if not killerFolder then return false end
    return killerFolder:FindFirstChild(character.Name) ~= nil
end

function AutoBlock.AttemptBlockForSound(sound)
    if not AutoBlock.Enabled then return end
    if not GetPlayerStyle or GetPlayerStyle() ~= "fighter" then return end

    local soundId = AutoBlock.ExtractNumericSoundId(sound)
    if not soundId or not AutoBlock.AttackSounds[tostring(soundId)] then return end
...
function AutoBlock.HookSound(sound)
    ...
    AutoBlock.SoundHooks[sound] = {playedConn, propConn, destroyConn}
...
function AutoBlock.InitializeSoundHooks()
    ...
    for _, model in ipairs(killerFolder:GetChildren()) do
        if model:IsA("Model") then
            AutoBlock.HookCharacterSounds(model)
        end
    end

    killerFolder.ChildAdded:Connect(function(model)
        if model:IsA("Model") and AutoBlock.Enabled then
            AutoBlock.HookCharacterSounds(model)
        end
    end)
end
...
function AutoBlock.CleanupSoundHooks()
    for _, hooks in pairs(AutoBlock.SoundHooks) do
...
function AutoBlock.Start()
    if AutoBlock.Enabled then return end
    ...
    AutoBlock.Connection = RunService.Heartbeat:Connect(function()
        if not AutoBlock.Enabled then return end
        if not Player.Character or not GetPlayerStyle or GetPlayerStyle() ~= "fighter" then return end

        local myHRP = Player.Character:FindFirstChild("HumanoidRootPart")
        if not myHRP then return end

        -- Re-init hooks ถ้าหาย
        local hasHooks = next(AutoBlock.SoundHooks) ~= nil
        ...
        -- ตรวจ Animation ก่อน (มีความสำคัญสูง)
        local shouldBlock, animType = AutoBlock.CheckKillerAnimation(killerModel, killerHRP, distance)
        if shouldBlock then
        ...
        for sound, _ in pairs(AutoBlock.ActiveSounds) do
            if sound and sound.Parent and sound.IsPlaying then
                if (not AutoBlock.SoundCooldowns[sound] or (now - AutoBlock.SoundCooldowns[sound]) > AutoBlock.Cooldown) then
                    if distance <= AutoBlock.MinDistance and AutoBlock.IsKillerLookingAtMe(killerHRP) then
...
function AutoBlock.SetMinDistance(value)
    AutoBlock.MinDistance = math.clamp(value, 5, 50)
end

function AutoBlock.SetThrowDistance(value)
    AutoBlock.ThrowDistance = math.clamp(value, 20, 150)
end
...
Auto:Slider({
    Title = "Parry Throw Range",
    Desc = "",
    Value = { Min = 20, Max = 150, Default = 80 },
    Callback = function(v)
        AutoBlock.SetThrowDistance(v)
    end
})

Auto:Toggle({
    Title = "Auto Parry (Fighter)",
    Desc = "",
    Value = false,
    Callback = function(v)
        if v then
